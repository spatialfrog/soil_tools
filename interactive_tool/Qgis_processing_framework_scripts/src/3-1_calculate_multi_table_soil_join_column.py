"""
purpose:
process the joined soil tables for single column calculation and csv output
...
joined table column for processing
...
##joined_soil_table=table
##join_table_soil_column_to_calculate=field joined_soil_table
##slc_shapefile=vector
##slc_shapefile_polygon_id_column=field slc_shapefile
##option_soil_join_table_slc_id_column=string sl
##option_soil_join_table_percent_column=string percent
...
inSoilDbPath = utils.getQgisTableLayerFilePathInfo(joined_soil_table)
# db instance
...
calculationColumnName = '"%s"' %(join_table_soil_column_to_calculate)

# convert full connection path of user selected table in qgis toc to actual table name
tableName = utils.getQgisTableLayerFilePathInfo(joined_soil_table, pathKey="table")
...
headers, results = db.calculateField(slcIds, dbSlcKey=option_soil_join_table_slc_id_column, tableName=tableName, columnName=calculationColumnName, dbPercentKey=option_soil_join_table_percent_column)
